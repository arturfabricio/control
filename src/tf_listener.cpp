...
void transformPoint(const tf::TransformListener &listener)
{
    ...
    camera_point.header.frame_id = "front_camera";
    ...
    try
    {
        ...
        ROS_INFO("front_camera: (%.2f, %.2f. %.2f) -----> base_link: (%.2f, %.2f, %.2f) at time %.2f",
                 camera_point.point.x, camera_point.point.y, camera_point.point.z,
                 base_point.point.x, base_point.point.y, base_point.point.z, base_point.header.stamp.toSec());
    }
    catch (tf::TransformException &ex)
    {
        ROS_ERROR("Received an exception trying to transform a point from \"front_camera\" to \"base_link\": %s", ex.what());
    }
}
